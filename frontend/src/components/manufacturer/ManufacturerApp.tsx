...
export function ManufacturerApp() {
  ...
  // Server data, not browser storage, decides whether setup is complete.
  useEffect(() => {
    if (!isLoaded || !isSignedIn || !orgId) return;
    if (!isManufacturerAccountPath(pathname) && !isManufacturerDashboardPath(pathname)) return;
    let cancelled = false;

    async function checkAccount() {
      try {
        const token = await getToken();
        const result = await api<{ account_exists: boolean }>(
          "/manufacturer/account",
          token,
        );
        if (cancelled) return;
        if (result.account_exists && isManufacturerAccountPath(pathname)) {
          router.replace(MANUFACTURER_DASHBOARD_PATH);
        } else if (!result.account_exists && isManufacturerDashboardPath(pathname)) {
          router.replace(MANUFACTURER_ACCOUNT_PATH);
        }
      } catch {
        // The form keeps its own actionable error for mutations. A temporary
        // read failure must not destroy the user's local form state.
      }
    }
  ...
  }, [getToken, isLoaded, isSignedIn, orgId, pathname, router]);
  ...
  async function handleAccountCreated(submission: AccountSubmission) {
    if (!createOrganization || !setActive) {
      throw new Error("Clerk organization creation is not available.");
    }
...
const token = await getToken({
  skipCache: true,
    ...
    await api("/manufacturer/account", token, {
      method: "POST",
      body: JSON.stringify({
        ...
        company_category: submission.companyType,
        country: submission.country,
        phone: submission.phone || null,
        production_capacity: submission.capacity || null,
      }),
    });
