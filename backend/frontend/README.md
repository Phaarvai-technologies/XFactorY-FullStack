...
Frontend for the X!Y manufacturing marketplace: Next.js (App Router), React,
TypeScript, Tailwind CSS, Clerk authentication, and FastAPI integration.
...
| `NEXT_PUBLIC_API_URL` | FastAPI base URL (`http://localhost:8000/api/v1`) |
...
TypeScript personas live in `src/types/personas.ts`. The role selection UI at
`/onboarding/roles` persists selections through FastAPI. These marketplace
personas remain separate from backend authorization permissions.
