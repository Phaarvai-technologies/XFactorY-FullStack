...
def verify_clerk_token(token: str, settings: Settings) -> dict:
    ...
    try:
        ...
        raise HTTPException(401, "Invalid or expired Clerk session token") from exc
...
async def get_actor(
    ...
    if row is None:
        ...
        user_id = row["id"]
   # Clerk session-token version 1 used `org_id`.
# Clerk session-token version 2 uses `o.id`.
